...
void MD5Transform	(size_t state[4], unsigned char block[64]);
void MD5Encode		(unsigned char *output, size_t *input, size_t len);
void MD5Decode		(size_t *output, unsigned char *input, size_t len);
...
void md5(char *source, size_t length, char *output) {
	...
	MD5Init		(&md5);
	MD5Update	(&md5, (unsigned char *) source, length);
